...
def simple_step():
    ...
    # HIGH PULSE
    set(pins, 1)       
    mov(x, osr)# Move the contents (i.e, No.of cycles) in OSR to Y register              
    label("delay_high")
...
while True:
    ...
    if width and 800 <= width <= 2200: #as 800 and 2200 are min and max pwm limits that any reciver or fc can output
        ...
        step_delay_cycles = int(get_step_delay_cycles(steps_per_second)) #convert to int 
        print(f"step_delay_cycles = {step_delay_cycles:.3f}")
    #----------------------------------------------------------------------------------------------------------------
        sm1.put(step_delay_cycles) #Accepts only integer values i.e, no.of cycles , change to increase or decrease the speed
