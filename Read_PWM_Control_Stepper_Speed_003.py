...
def get_steps_per_second_with_gear_box(safe_target_rpm) -> float:
    ...
    Calculating Steps Per Second (SPS) 
        we know, rps = no.of Rotations / second
            To Complete 1Rotation (for Stepper with out gear box) = MICRO_STEPS (3200 SPR-Steps Per Rotation)
            i.e, 1Rotation = 3200 Steps 
        SPS = safe_target_rps * MICRO_STEPS ----> With out any gear box
        SPS with Gear Box
            To Complete 1Rotation (for Stepper with gear box) = MICRO_STEPS * GEAR_RATIO (3.71 (if gear ratio is 1:3.71))
            i.e, 1Rotation = 3200 * 3.71
                           = 11872 Steps
        SPS = safe_target_rps * MICRO_STEPS * GEAR_RATIO
    '''
    safe_target_rps = safe_target_rpm / 60
