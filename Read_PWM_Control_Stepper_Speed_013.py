...
'''
'''
Version 15:- Final Dual-PIO Non-Blocking Code with Mechanical Helper Functions
'''
...
FOLDING_ANGLE_DEGREES = 4*360.0 # Desired fold angle 
TARGET_PIPE_RPM = 5.0       # Target output speed in RPM Never go above 50RPM@200 Steps/rev
...
# --- New Pipe Folding Helper Functions ---
def get_pipe_steps_for_angle(angle_deg, gear_ratio, micro_steps):
    """Calculates exact steps needed for a specific angle, factoring in the gearbox"""
    steps_per_360 = micro_steps * gear_ratio
    required_steps = (angle_deg / 360.0) * steps_per_360
    return int(required_steps)

def get_pipe_delay_for_rpm(target_rpm, gear_ratio, micro_steps):
    """Calculates the PIO delay cycles to hit the target RPM on the output shaft"""
...
# --- Auto-calculate Pipe Movement based on Mechanical Settings ---
...
print(f"Calculated Pipe Steps for {FOLDING_ANGLE_DEGREES} deg: {STEPS_TO_MOVE}")
print(f"Calculated PIO Delay for {TARGET_PIPE_RPM} RPM: {PIPE_STEP_DELAY_CYCLES} cycles")
