...
pwm_pin = 17 #GPIO17
pwm_pin_pipe_folding = 21 #GPIO21

MIN_RPM = 10.0 # Tested till 10RPM Dont go Under This 
MAX_RPM = 80 # Tested till 400RPM With Gear Box (Don't Go beyond 400RPM)
...
GEAR_RATIO = 3.71
#If their is no gear box keep 1.0
                  #If gearbox is used example:- 1:3.71 gear ratio "GEAR_RATIO = 3.71"

MAX_PWM_PIPE_FOLDING = 1951.0
MIN_PWM_PIPE_FOLDING = 1051.0
MID_PWM_PIPE_FOLDING = 1501.0

MICRO_STEPS_PIPE_FOLDING = 200
GEAR_RATIO = 1.0

SET_RPM_PIPE_FOLDING = 5.0
STEPS_TO_FOLD = 50 #Need to check and change

step_pin = Pin(2, Pin.OUT) #GPIO2
dir_pin  = Pin(3, Pin.OUT) #GPIO3
ena_pin  = Pin(4, Pin.OUT) #GPIO4

step_pin_pipe_folding = Pin(5,Pin.OUT)
dir_pin_pipe_folding = Pin(6,Pin.OUT)
ena_pin_pipe_folding = Pin(7,Pin.OUT)

dir_pin.value(1) #1-> cw, 0->ccw
ena_pin.value(0) #0-> disable driver, 1-> enable driver

dir_pin_pipe_folding.value(1)
ena_pin_pipe_folding.value(1) #For Pipe Folding always be enabled

# --- New Variables for Stepper 2 State Management ---
# Define states
STATE_UNKNOWN = 0
STATE_FOLDED = 1   # Corresponds to ~1000us
STATE_UNFOLDED = 2 # Corresponds to ~2000us
STATE_MID = 3      # Corresponds to ~1500us

current_pipe_state = STATE_UNKNOWN
target_steps = 50
step_delay_ms = 2 # Adjust this for the speed of the folding stepper (2ms = relatively fast)

#-------------------------------------------- ASM Code For PWM Reading (Seed Picker)----------------------------------------------
...
#-------------------------------------------- ASM Code For PWM Readingm (pipe folding)----------------------------------------------
@rp2.asm_pio() #This is The Decorator and Function Under This is for State Machine in PIO Assmebly Instructions
def count_high_pulse_width_of_pwm_pipe_folding():
    wrap_target() #All Instructions Below target till wrap() acts like a continous loop; But if any instructions above targer are only executed once  
    wait(0,pin,0) # wait(HIGH or LOW, source , pin index) ; source can be directly a GPIO Number or pin 
    set(x,0)      #set X = 0 (Counter , Only decrement it)
    wait(1,pin,0) 
...
#creating an object "sm0" of state machine we have 12 in total for pi pico 2w
sm2 = rp2.StateMachine(2, count_high_pulse_width_of_pwm_pipe_folding, freq=PWM_READ_FREQ, in_base = pwm_pin_pipe_folding, jmp_pin = pwm_pin_pipe_folding)

sm2.active(1) # "1" for activating the state machine and "0" for deactivating the state machine
#----------------------------------------------------------------------------------------------------------------------------------------------------

#------------------------------------- ASM Code For Stepper (pipe Folding) -------------------------------------------------------
@rp2.asm_pio(set_init=rp2.PIO.OUT_LOW) #initilize the pins to OUT and with LOW Logic
def simple_step_pipe_folding():
...
sm3 = rp2.StateMachine(3, simple_step_pipe_folding, freq=STEPPER_FREQ, set_base=step_pin_pipe_folding)
...
def get_pulse_width_us_pipe_folding()->float:
    if sm2.rx_fifo():#Check if rx fifo has some values (it can hold up to 4 values)
        while sm2.rx_fifo() > 1: #True if rx fifo has more than 1 value; false if it has only 1 
            sm2.get() #Clears 2nd , 3rd and 4th values in rx fifo
        raw = sm2.get() #only returns the 1st value in the rx fifo as remaining are cleared in above while condition
        counts = (0xFFFFFFFF - raw) & 0xFFFFFFFF 
        return (counts * 2)/125 # 2 for no.of instructions(only cycle consumable) and 125 for pwm read frequency in "Mhz" 125_000_000 Hz = 125Mhz
...
def get_steps_per_second_with_gear_box_pipe_folding() -> float:
    safe_target_rps_pipe_folding = SET_RPM_PIPE_FOLDING / 60
    return safe_target_rps_pipe_folding * MICRO_STEPS_PIPE_FOLDING * GEAR_RATIO_PIPE_FOLDING
...
# --- Helper Function for Stepper 2 Movement ---
def move_stepper_2(steps, direction):
    dir_pin_pipe_folding.value(direction) # 1 or 0 depending on your wiring for CW/CCW
    for _ in range(steps):
        step_pin_pipe_folding.value(1)
        time.sleep_ms(step_delay_ms)
        step_pin_pipe_folding.value(0)
        time.sleep_ms(step_delay_ms)
...
while True:
    # --- 1. Handle Main Stepper (Speed Control) ---
    ...
    # --- 2. Handle Secondary Stepper (Position Control) ---
    width_pipe_folding = get_pulse_width_us_pipe_folding()
    # print(f"Pipe PWM: {width_pipe_folding}") # Uncomment for debugging

    if width_pipe_folding:
        # Check for Fold Command (~1051)
        if 1000 <= width_pipe_folding <= 1100:
            if current_pipe_state != STATE_FOLDED:
                print("Command: FOLD (CCW)")
                move_stepper_2(target_steps, 0) # Assume 0 is CCW
                current_pipe_state = STATE_FOLDED

        # Check for Unfold Command (~1951)
        elif 1900 <= width_pipe_folding <= 2000:
            if current_pipe_state != STATE_UNFOLDED:
                print("Command: UNFOLD (CW)")
                move_stepper_2(target_steps, 1) # Assume 1 is CW
                current_pipe_state = STATE_UNFOLDED

        # Check for Mid Command (~1501)
        elif 1450 <= width_pipe_folding <= 1550:
            if current_pipe_state != STATE_MID:
                 print("Command: STOP (MID)")
                 # We don't need to move, just update state so we don't re-trigger
                 # The stepper driver will hold position inherently if enabled.
                 current_pipe_state = STATE_MID

    time.sleep(0.05) # Main loop delay (reduced slightly to catch switch changes faster)
