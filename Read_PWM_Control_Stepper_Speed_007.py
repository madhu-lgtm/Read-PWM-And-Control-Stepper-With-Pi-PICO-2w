...
3) Stepper       = Nema 17 JK42HS60-1684
4) Gear Box      = NO
5) Driver        = TB6600 
6) Current Set   = 1.5Amp
7) Micro Step    = 3200
------------------------------------------------------------------
...
ena_pin.value(1) #0-> disable driver, 1-> enable driver
...
def simple_step():
    '''
    as we put the value "sm1.put(int)" for every 100ms , between this loop delay tx fifo does not contain any thing ,
    we will make x to 0 and will read the OSR for any new value and move it to x, and check x
    if x found to be zero we must proceed with previous non zero value stored in y
    if x found to be non zero value we must update the y register for new number of counts.
    
    1) wait until we have somthing in OSR , when found somthing pull
    2) copy the osr value (no.of counts) to the y register
    3) set the pulse pin HIGH                ---> HIGH
    4) copy the y value to x                 ---> HIGH
    5) decrement the x till it becomes zero  ---> HIGH
    6) set the pulse pin LOW                 ---> LOW
    7) copy the y value to x                 ---> LOW
    8) decrement the x till it becomes zero  ---> LOW
    
    9) set x to zero
    10) pull(noblock)
    11) copy the contents of osr to x
    12) Check x
    13) if x zero; dont update the y
    14) if x is not zero , update the y
    15) loop back to 
    '''
...
def get_compensated_step_delay_cycles(step_delay_cycles)->float:
    '''
    1) For TB6600 Stepper Driver when you connect the pul- pin , the stepper motor steps on falling edge
        if at all connected to pul+ pin, stepper motor steps on raising edge
    2) So For one step we are consuming Half the frequency i.e, => step_delay_cycles/2
    3) And Also we are consuming 8 instructions for HIGH or LOW pulse generation => (step_delay_cycles/2) - 8
        for HIGH :- set(pins, 1)  [5]->set(pins, 1),set(pins, 1),set(pins, 1),set(pins, 1),set(pins, 1),set(pins, 1) ; mov(x, y); jmp(x_dec, "delay_high")
        for LOW  :- set(pins, 0); mov(x, y); jmp(x_dec, "delay_low"); set(x,0); pull(noblock); mov(x,osr); jmp(not_x,"keep_old"); mov(y,osr)
    '''
...
while True:
    ...
    #----------------------------------------------------------------------------------------------------------------
        sm1.put(compensated_step_delay_cycles) #Accepts only integer values i.e, no.of cycles , change to increase or decrease the speed
                                   #Checked range is 50 to 4000 for 42HS60 -1684
