'''
Version 14:- Tested Stop Feature for pipe folding stepper
Version 13:- Included helper functiuons for stepper 2
...
--------------- Components; Please check circuit dia ---------------------------------------
...
Code Updated On :- 07-Mar-2026
Code Developed By :- K Madhu Mohan Chary
Contact Details   :- madhu@marutdrones.com

'''
'''
Version 16:- Final Dual-PIO Non-Blocking Code with Absolute Position Tracking
'''
...
# --- Pins ---
pwm_pin = 17            # Main Stepper PWM (Seed Picker)
...
ena_pin_pipe.value(1) # 1 -> ENABLE for TB6600 common anode wiring

# ==============================================================================
#                      MAIN STEPPER SETTINGS (Seed Picker)
# ==============================================================================
MIN_RPM = 10.0
MAX_RPM = 80 
MAX_PWM = 1951.0 
...
# ==============================================================================
#                    PIPE FOLDING STEPPER SETTINGS (Mechanical)
# ==============================================================================
MICRO_STEPS_PIPE = 200       # Pulses per revolution on the driver
GEAR_RATIO_PIPE = 1.0        # Change this if using a gearbox (e.g., 3.71, 5.18)
FOLDING_ANGLE_DEGREES = 90 # Desired fold angle 
TARGET_PIPE_RPM = 1.0       # Target output speed in RPM
...
# ---------------- ASM Position Stepper (Step Counting) ----------------
@rp2.asm_pio(set_init=rp2.PIO.OUT_LOW)
def position_step():
...
# --- Pipe Folding Helper Functions ---
def get_pipe_steps_for_angle(angle_deg, gear_ratio, micro_steps):
    steps_per_360 = micro_steps * gear_ratio
    return int((angle_deg / 360.0) * steps_per_360)

def get_pipe_delay_for_rpm(target_rpm, gear_ratio, micro_steps):
    motor_rpm = target_rpm * gear_ratio
    steps_per_second = (motor_rpm / 60.0) * micro_steps
    step_delay_cycles = STEPPER_FREQ / steps_per_second
    return int(get_compensated_step_delay_cycles(step_delay_cycles))

# --- Auto-calculate Pipe Movement ---
STEPS_TO_MOVE = get_pipe_steps_for_angle(FOLDING_ANGLE_DEGREES, GEAR_RATIO_PIPE, MICRO_STEPS_PIPE)
PIPE_STEP_DELAY_CYCLES = get_pipe_delay_for_rpm(TARGET_PIPE_RPM, GEAR_RATIO_PIPE, MICRO_STEPS_PIPE)

print("--- System Initialized ---")
print("WARNING: Ensure pipes are physically UNFOLDED when powering on!")
print(f"Calculated Target Steps: {STEPS_TO_MOVE}")

# --- Absolute Position Tracking Variables ---
current_absolute_steps = 0  # 0 = Fully Unfolded, STEPS_TO_MOVE = Fully Folded
last_commanded_steps = 0
last_move_direction = -1    # 0 = CCW (Fold), 1 = CW (Unfold)
is_pipe_moving = False
old_cycle_count = 0 

# MAIN LOOP
while True:
    # --- 1. Main Stepper Logic (Seed Picker) ---
    ...
    if width_pipe:
        target_state = current_pipe_state
        if 1000 <= width_pipe <= 1100:
            target_state = STATE_FOLDED
        elif 1900 <= width_pipe <= 2000:
            target_state = STATE_UNFOLDED
        elif 1450 <= width_pipe <= 1550:
            target_state = STATE_MID

        # Only run this block if the switch position actually changed
        if target_state != current_pipe_state:
            
            # --- INTERROGATE PIO & UPDATE ABSOLUTE POSITION ---
            if is_pipe_moving:
                # Flush old junk from the queue
                while sm5.rx_fifo(): sm5.get() 
                
                # Ask PIO: "How many steps are left in your countdown?"
                sm5.exec("push()") 
                time.sleep_ms(1)
                
                remaining_steps = 0
                if sm5.rx_fifo():
                    remaining_steps = sm5.get()
                
                # Math: Steps Command - Steps Left = Steps Taken
                steps_taken = last_commanded_steps - remaining_steps
                if steps_taken < 0: steps_taken = 0
                
                # Update our absolute position
                if last_move_direction == 0: # We were folding
                    current_absolute_steps += steps_taken
                elif last_move_direction == 1: # We were unfolding
                    current_absolute_steps -= steps_taken
                    
                # Clamp the values to prevent math errors
                current_absolute_steps = max(0, min(STEPS_TO_MOVE, current_absolute_steps))
                is_pipe_moving = False

            # --- KILL-SWITCH (Instantly halt current move) ---
            sm5.active(0)
            sm5.init(position_step, freq=STEPPER_FREQ, set_base=step_pin_pipe)
            sm5.active(1)

            # --- EXECUTE NEW COMMAND BASED ON ABSOLUTE POSITION ---
            if target_state == STATE_FOLDED:
                steps_needed = STEPS_TO_MOVE - current_absolute_steps
                if steps_needed > 0:
                    dir_pin_pipe.value(0) # CCW
                    time.sleep_ms(2)      # Hardware delay for TB6600
                    sm5.put(PIPE_STEP_DELAY_CYCLES) 
                    sm5.put(steps_needed)
                    
                    last_commanded_steps = steps_needed
                    last_move_direction = 0
                    is_pipe_moving = True
                    print(f"Pipe: Folding {steps_needed} steps. Target: Folded")
                else:
                    print("Pipe: Already fully folded!")

            elif target_state == STATE_UNFOLDED:
                steps_needed = current_absolute_steps # Return to 0
                if steps_needed > 0:
                    ...
                    is_pipe_moving = True
                    print(f"Pipe: Unfolding {steps_needed} steps. Target: Unfolded")
                else:
                    print("Pipe: Already fully unfolded!")

            elif target_state == STATE_MID:
                print(f"Pipe: Stopped. Absolute Position: {current_absolute_steps}/{STEPS_TO_MOVE} steps")

            current_pipe_state = target_state
